...
class Sonido(var actividad: Activity, var grabacion: MediaRecorder= MediaRecorder(), var rutaAudio:String="" ) {
    ...
    fun grabandoAudio(grabacion:MediaRecorder)
    {
        ...
        val ficheroAlmacenamientoAudio=
            Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MUSIC).absolutePath+"/AudioVocLearn"+ CRUDEntradas.nuevoId()+".mp3"
        ...
        fun preparacionAudio(reproducirAudio: MediaPlayer, idEntrada: Int) {
            try {
                reproducirAudio.setDataSource(
                    ...
                    ).absolutePath + "/AudioVocLearn" + idEntrada + ".mp3"
                )
