    ...
    companion object {
        ...
        fun hacerCopiaSeguridad(real :Realm?, act:Activity) {
            ...
            try {
                ...
                val ficheroDestino = File(
                    PATH_TO_WRITE,
                    FILE_TO_WRITE
                )
                //FICHERO REALM INTERNO (LOCAL)
                val ficheroOrigen =
                    File(act.getApplicationContext().getFilesDir(),
                        FILE_TO_RECOVERY
                    )
        ...
        fun restaurarCopiaSeguridad(act:Activity) {
            ...
            //FICHERO REALM INTERNO (LOCAL)
            val ficheroDestino = File(act.getApplicationContext().getFilesDir(),
                FILE_TO_RECOVERY
            )
            //FICHERO REALM EXTERNO (COPIA SEGURIDAD)
            val ficheroOrigen = File(
                PATH_TO_WRITE,
                FILE_TO_WRITE
            )
        ...
        fun permisosSiNo(act: Activity, explicacion:String, permisos:Array<String>, codigoPermiso:Int) {
            ...
            if (shouldShowRequestPermissionRationale(act, permisos[0])) {
                ...
                AlertDialog.Builder(
                    ...
                    //MICROFONO
                AlertDialog.Builder(ContextThemeWrapper(act,
                    R.style.AlertDialog
                ))
                    .setTitle(R.string.necesidadPermisos)
        ...
        {
            return File(act.getApplicationContext().getFilesDir(),
                FILE_TO_RECOVERY
            ).exists()
        }
